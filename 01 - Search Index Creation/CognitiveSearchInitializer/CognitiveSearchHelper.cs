...
using CognitiveSearchInitializer.Models;
    ...
    public static class CognitiveSearchHelper
    {
        ...
        public static async Task<DataSource> GetOrCreateBlobDataSource(ISearchServiceClient serviceClient, string name, DataSourceType dataSourceType, BlobStorageConfig blobStorageConfig, string query = "")
        {
            ...
            var dataSource = new DataSource
            {
                ...
                Container = new DataContainer(blobStorageConfig.SourceContainerName)
            };
        ...
        public static async Task<Skillset> GetSkillset(ISearchServiceClient serviceClient, string skillsetName, AppConfig appConfig)
        {
            ...
            else
            {
                ...
                return await GetSkillsetFromFile(skillsetName, appConfig);
            }
        }

        public static async Task<Skillset> GetSkillsetFromFile(string name, AppConfig appConfig)
        {
            using (var reader = new StreamReader($"PipelineJson/{name}.json"))
            {
                var json = await reader.ReadToEndAsync();
                json = json.Replace("[AzureFunctionEndpointUrl]", appConfig.FunctionApp.Url);
                json = json.Replace("[AzureFunctionDefaultHostKey]", appConfig.FunctionApp.DefaultHostKey);
                json = json.Replace("[BlobContainerName]", appConfig.BlobStorage.ExtractionContainerName);
                ...
                skillset.CognitiveServices = new CognitiveServicesByKey(appConfig.CognitiveServices.Key, appConfig.CognitiveServices.ResourceId);
