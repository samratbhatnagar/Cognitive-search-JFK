...
const mapSearchResponseForResults = (
  ...
  return isArrayEmpty(response.value)
    ? null
    : response.value.map(r => mapResultToItem(r, responseConfig));
};
...
const mapSearchResponseForFacets = (
  ...
  return isArrayEmpty(response.facets)
    ...
    : baseFacets
        .map(bf =>
          mapResponseFacetToViewFacet(
            response.facets.find(rf => rf.fieldName === bf.fieldId),
            bf
          )
        )
