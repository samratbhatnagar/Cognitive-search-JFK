...
export const mapSuggestionResponseToState = (
  state: State,
  response: AzResponse,
  ...
  return {
    ...state,
    suggestionCollection: isArrayEmpty(response.results)
      ? null
      : response.results.map(s => mapSuggestionResponse(s))
  };
};

// [Suggestion] FROM view model TO AzApi.

export const mapStateToSuggestionPayload = (
  state: State,
  config: ServiceConfig
): AzPayload => {
